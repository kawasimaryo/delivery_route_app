...
### ルート最適化
![ルート最適化](docs/route.png)

ルート最適化ボタンをクリックすると、Directions APIで最短経路を自動計算し、地図上に青線でルートを描画。
...
- **ルート最適化** - Google Maps Directions APIで最短経路を自動計算
- **ジオコーディング** - 住所から座標を自動変換
...
| CSS | バニラCSS（CSS変数使用） |
| 認証 | Devise |
| 外部API | Google Maps JavaScript API / Geocoding API / Directions API |
| インフラ | Heroku対応 |
...
### 地図・ルート機能
- Google Mapsでの配達先マーカー表示（ステータス別色分け）
- ルート最適化（Directions APIで最短経路を自動計算）
- ルート描画（配達先間の経路を地図上に青線で表示）
- ジオコーディング（住所→座標の自動変換）
...
| `GOOGLE_MAPS_API_KEY` | 地図表示用 | 地図機能使用時 |
| `GEOCODING_API_KEY` | ジオコーディング・ルート最適化用 | 地図機能使用時 |
